...
public class AntigravityHookCommandTests {
    ...
    public async Task Missing_event_exits_zero_without_touching_network() {
        ...
        var rc = await new AntigravityHookCommand(Config.Root, Resolutions.At("http://127.0.0.1:0", Config.Root), new HookClock(TimeProvider.System)).Handle(["hook", "--antigravity"], new StringReader(""), new StringWriter());
    ...
    public async Task PreInvocation_with_non_string_fields_fails_open() {
        ...
        var rc = await new AntigravityHookCommand(Config.Root, Resolutions.At("http://127.0.0.1:0", Config.Root), new HookClock(TimeProvider.System)).Handle(["hook", "--antigravity", "PreInvocation"],
    ...
    public async Task Non_PreInvocation_events_are_no_ops(string ev) {
        ...
        var rc = await new AntigravityHookCommand(Config.Root, Resolutions.At("http://127.0.0.1:0", Config.Root), new HookClock(TimeProvider.System)).Handle(["hook", "--antigravity", ev],
    ...
    public async Task PreInvocation_without_conversation_or_transcript_is_a_no_op() {
        ...
        await Assert.That(await new AntigravityHookCommand(Config.Root, Resolutions.At("http://127.0.0.1:0", Config.Root), new HookClock(TimeProvider.System)).Handle(["hook", "--antigravity", "PreInvocation"],
