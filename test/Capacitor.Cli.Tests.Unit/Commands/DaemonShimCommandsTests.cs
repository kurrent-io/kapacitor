...
/// install arm is driven through the injected seam so the verb's wire contract is testable without
/// a real admin prompt.
/// </summary>
[NotInParallel] // every test captures Console
public class DaemonShimCommandsTests {
    ...
    static async Task<(int Exit, ShimEnsureJson? Json, string? Out)> Run(
            ...
            Func<string, CancellationToken, Task<ShimResult>>? install = null, bool isMacOs = false) {
        using var capture = ConsoleOutput.StartFullCapture();
        var exit = await DaemonShimCommands.Ensure(args, resolveTarget: () => target,
            probe: new FakeProbe(onPath), install: install, isMacOs: isMacOs);
