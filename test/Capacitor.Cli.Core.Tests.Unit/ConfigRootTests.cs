...
public class ConfigRootTests {
    ...
    // The fallback resolves the home from the environment, so the ephemeral home has to be reached
    // that way too: an injected value cannot change what FromEnvironment reads.
    [Test, NotInParallel]
    public async Task Default_directory_lives_under_the_config_folder() {
        ...
        using var home = EnvScope.Exclusive("HOME", Home.Path);

        await Assert.That(ConfigRoot.FromEnvironment().Directory)
                    .IsEqualTo(Path.Combine(Home.Path, ".config", "kcap"));
    }

    [Test, NotInParallel]
    public async Task Environment_value_wins_over_the_home_fallback() {
        using var home = EnvScope.Exclusive("HOME", Home.Path);
        var fallback   = Path.Combine(Home.Path, ".config", "kcap");
