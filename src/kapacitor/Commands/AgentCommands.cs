...
public static class AgentCommands {
    ...
    static int StartDetached(string[] args) {
        ...
        // Use ArgumentList for safe argument passing (no quoting issues)
        var psi = new ProcessStartInfo {
            FileName = exePath,
            UseShellExecute = false,
            // Don't redirect — avoids pipe buffer deadlock in detached mode
    ...
    static int PrintUsage() {
        Console.Error.WriteLine("Usage: kapacitor agent <start|stop|status>");
        ...
        Console.Error.WriteLine("  -d, --detach          Run in background");
        return 1;
    }
}
