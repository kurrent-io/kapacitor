...
public static class SessionStatusDots {
    ...
    /// beside the dot vocabulary so the two can never disagree. A finished turn counts only for an
    /// agent the user can answer: a flow participant between rounds waits on the flow.
    public static bool NeedsAttention(AgentStatusDto dto) =>
        dto.Status == "Failed" || (dto.AwaitingInput == true && !AgentActionService.IsProtectedKind(dto.Kind));
