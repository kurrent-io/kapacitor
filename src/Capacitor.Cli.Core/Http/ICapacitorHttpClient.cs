...
public interface ICapacitorHttpClient {
    ...
    /// afford to retry — asks for a memory client instead.</para>
    /// </summary>
    ...
    /// context, so a hop must not be able to substitute one — least of all a cross-origin hop, which
    /// arrives at the substitute with the credential already stripped.
    /// </summary>
    Task<HttpClient> ForMemoryAsync(CancellationToken ct = default);
