...
public static class CapacitorClients {
    ...
    /// <summary>The session-start memory index: the default chain, except a 3xx is not followed. What
    /// comes back is injected into the agent's context, so a hop must not be able to substitute it.</summary>
    public const string Memory = "capacitor-memory";
