...
public static class HarnessCatalog {
    ...
    public static readonly IReadOnlyList<KnownHarness> All = [
        ...
        new("kiro", "Kiro", "--kiro", r => r.Kiro,
            i => KiroHooksInstaller.IsInstalled(KiroPaths.KcapAgentJson(i.Home))),
        new("pi", "Pi", "--pi", r => r.Pi,
            i => PiExtensionInstaller.IsInstalled(PiPaths.KcapExtension(i.Home))),
        new("opencode", "OpenCode", "--opencode", r => r.OpenCode,
            i => OpenCodeExtensionInstaller.IsInstalled(OpenCodePaths.KcapPlugin(i.Home, i.XdgConfigHome))),
