...
/// cap: the daemon is local, and a wedged one must not eat the hook's budget.
/// </summary>
internal static class DaemonInputWaitRelay {
    internal static readonly TimeSpan Cap = TimeSpan.FromSeconds(1);

    public static async Task NotifyAsync(string vendor, string? sessionId, string? cwd, bool waiting) {
