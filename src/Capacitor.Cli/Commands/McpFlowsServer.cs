        ...
        new(
            ...
            new(
                ...
                new() {
                    ...
                    ["context"]      = new("string", "Background context for the reviewer: what to focus on, constraints, definition of done. State where the changes live — the reviewer sees a mirror of THIS SESSION's project directory (the git root resolved for the session), which is not necessarily the directory you are working in now; if the changeset is elsewhere (you changed directory, another worktree, another repo) or incomplete there, say so, give an explicit commit range, and inline the relevant diffs. Whether it sees your UNCOMMITTED work is conditional: only when the run actually borrows your checkout. Responses report this as workspace: borrowed | fallback (<reason>) | unknown — for the reserved review aliases; other flow kinds report unknown. On fallback your checkout was NOT borrowed, so inline anything uncommitted that matters."),
                    ["instructions"] = new("string", "Optional additional instructions for the reviewer agent."),
                    ["mode"]         = new("string", "Optional. Pass 'context-only' to have the reviewer treat the submitted context/diff as authoritative rather than reading the repository. By default, when it runs on the same machine, the reviewer runs in a worktree mirrored from THIS SESSION's project directory — which is not necessarily the directory you are working in — so it can ground the review in the actual source; uncommitted changes are included only when your checkout is actually borrowed. Passing 'context-only' opts out of that."),
        ...
        new(
            ...
            new(
                ...
                new() {
                    ...
                    ["target_ref"]     = new("string", "A reference to the target (PR URL, branch name, file path, etc.)."),
                    ["target_title"]   = new("string", "Human-readable title for the target (PR title, spec name, etc.)."),
                    ["context"]        = new("string", "Background context for the agent: what to focus on, constraints, definition of done. State where the changes live — the participant sees a mirror of THIS SESSION's project directory (the git root resolved for the session), which is not necessarily the directory you are working in now; if the changeset is elsewhere (you changed directory, another worktree, another repo) or incomplete there, say so, give an explicit commit range, and inline the relevant diffs."),
                    ["instructions"]   = new("string", "Optional additional instructions for the agent."),
                    ["mode"]           = new("string", "Optional. Pass 'context-only' to have the agent treat the submitted context/diff as authoritative rather than reading the repository. By default, when it runs on the same machine, the agent runs in a worktree mirrored from THIS SESSION's project directory — which is not necessarily the directory you are working in — so it can ground the work in the actual source; uncommitted changes are included only when your checkout is actually borrowed. Passing 'context-only' opts out of that."),
