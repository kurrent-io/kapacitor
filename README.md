...
| Antigravity | yes | yes (`PreInvocation` → `{"injectSteps":[{"userMessage":…}]}`; zero bytes when there is no index) | **certified — interactive CLI** (a real `agy` 1.1.11 session on 2026-08-07 carried the injected index as its own transcript event). **Print mode (`agy -p`) does NOT inject**: the hook fires (the run is captured) but agy discards the returned `injectSteps` — verified by transcript comparison on 1.1.10 and 1.1.11, an upstream print-mode gap, not a kcap defect. The gated live cert (`KCAP_ANTIGRAVITY_MEMORY_LIVE=1` + `KCAP_URL`) drives `-p` and is retained as the upstream-change watch: it fails today by design and a pass means agy fixed print mode — update this row when it does. The Antigravity GUI app shares the same plugin and is untested | available — **interactive CLI** (certified); print mode receives nothing (upstream); injects **once per conversation** despite `PreInvocation` firing every invocation |
