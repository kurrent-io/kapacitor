...
Discovery needs an interactive terminal: SSO signs in through a `127.0.0.1` browser callback, which a browser on another machine can't reach. In SSH / headless environments discovery stops before signing you in and points you at the two routes that work — create a workspace in a browser and then run `kcap setup <tenant>`, or pass `--server-url <url>` for a workspace you already have.
