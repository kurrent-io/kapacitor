...
>
> This matters most for review flows. If `start_review_flow` is missing its `vendor` parameter, the agent driving it cannot name a reviewer, and the server falls back to its configured `Flows:Review:DefaultVendor` — so asking for "a review by Claude" can quietly get you a different reviewer. The kcap flow skills instruct agents never to claim a named reviewer ran when they could not send `vendor`, and reserved review aliases fail closed against a stale client (`client_upgrade_required`, `flow_client_protocol_required`, `flow_client_protocol_unsupported`) rather than silently substituting one.
>
