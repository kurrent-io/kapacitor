...
> **DeepSeek Harness (`dsh`)** is an **experimental spike** (AI-2020). Ingest is event-sourced: dsh writes its own `session.jsonl` (its `SessionEvent` stream), which the watcher tails directly (`vendor=dsh`) and `kcap import --dsh` replays from `~/.deepseek/harness/sessions/<id>/session.jsonl` (override with `KCAP_DSH_HOME`). Live capture requires a dsh Cordis plugin that shells `kcap hook --dsh --event session-start|session-end …`; auto-install (`kcap plugin install --dsh`) and setup-wizard detection are **not wired yet** pending confirmation of dsh's plugin API — a hand-installed plugin invoking that contract works end-to-end today.
