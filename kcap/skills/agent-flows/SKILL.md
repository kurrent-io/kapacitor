...
9. **State where your changes live.** The participant's worktree is mirrored from **your session's project directory** — the git root the flows MCP server resolved for this session — which is **not necessarily the directory you are working in now**. If you changed directory, are a subagent working in a different checkout, or the changeset lives in another worktree, another repository, or on another machine, the participant will not see it, and no tool parameter can redirect it. If any part of the changeset lives elsewhere (another git worktree, another repository, a different machine) or is not in that tree, say so explicitly in `context`/`message` and inline the relevant diffs or file contents — or pass `mode: "context-only"` so the participant treats your context as the sole source of truth. The participant is instructed to flag referenced changes it cannot find in its worktree; incomplete context wastes a full round.
