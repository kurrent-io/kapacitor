...
| 6 | **Zero wire-protocol changes; one deliberate daemon exit-code change** (amended 2026-08-10 from "zero daemon changes"): a server `NameInUse` rejection exits **0**. It is a deliberate refusal, not a crash — and under `KeepAlive SuccessfulExit=false` the old non-zero exit made a unit whose name is registered from another machine respin forever, past any verification window (the daemon opens its local socket before `ServerConnection` connects, so no post-hello check can outwait it). Plus one additive client-side change in `Capacitor.Cli.Core`: the hello reply's `DaemonVersion` propagates `CycleOutcome` → `LocalControlEvent.Unreachable` → `AttachStatus`, deduped on `(reason, daemonVersion)` (§4.3). |
