...
| # | Decision |
|---|----------|
| 1 | **Everything through the CLI (approach A).** The app shells the resolved `kcap` for every service operation and for detection. No plist/launchctl knowledge in the app. The CLI changes in this slice: (a) `daemon service status --json` (§3.4); (b) behavioral hardenings the lifecycle guarantees require (§3.4) — `service status` queries launchd even when the plist is absent (an orphaned loaded job must be visible); `service uninstall` distinguishes benign label absence from a failed `bootout`; `service stop` unloads the label (bootout, plist retained) instead of only signaling the process; `service start` bootstraps an unloaded label, else kickstarts. Rejected: moving service machinery into `Capacitor.Cli.Core` for in-proc use; hybrid in-proc reads. |
...
| 7 | **Every CLI child the app spawns runs with a pinned profile** (`KCAP_PROFILE` env overlay; `--profile` additionally on `service install`) **and, when known, the terminal PATH** (§3.6). Without the pin, the child's repo-aware profile resolution can capture a different tenant into the unit; without the PATH, `ServiceEnvironment.Capture` bakes the GUI app's minimal launchd PATH into the unit — a daemon that cannot find `claude`/`codex`. Because the baked PATH is load-bearing, **unit-writing mutations require a known terminal PATH** (unknown → no silent install; dialoged installs disclose the degraded PATH and proceed only with that consent). Starting an existing unit is exempt: it recaptures nothing — the unit's PATH was fixed at its install. |
...
- `daemon_pid`: the name's PID-file owner, **live identity-validated** the way `daemon stop` validates before killing (the `IsOurDaemon` start-token check that rejects dead, recycled, and foreign PIDs) — a raw parsed number would let PID reuse satisfy ownership, or falsely suppress auto-start as "coexistence". Null when absent, unusable, or failing validation. `job_pid == daemon_pid` (both non-null) is the **ownership** predicate: the launchd job *is* the daemon holding the name.
- **`service uninstall` hardening:** on a non-zero `bootout` the label is re-queried. Confirmed absent → benign: an already-unloaded label is an idempotent success and the plist is deleted (this is what lets the reinstall affordance run its uninstall-first sequence over a present-but-unloaded plist — `bootout` of an absent label exits non-zero). Still present or unknown → the plist is **retained** and the command exits non-zero — never a deleted file over a live KeepAlive job silently reported as success.
- **`service stop`/`service start` hardening:** stop performs `bootout` (label unloaded, plist retained — re-loaded at next login or by `service start`); `launchctl kill SIGTERM` cannot stop a lock-losing KeepAlive job that is between short-lived incarnations when the signal lands, so a signal-only stop is not a stop. start bootstraps the label when unloaded, else kickstarts.
...
- **Detach-on-cancel is scoped to `daemon start -d` and read-only queries**: cancellation abandons the *wait*, never the child — a detached daemon must survive the app quitting (existing behavior, kept there and only there).
- **Service mutations are never abandoned mid-flight**: app shutdown defers — bounded by the operation's own deadlines (§4.2) — until the in-flight mutation completes its verification or rollback. Otherwise a `service install` could finish bootstrapping after the controller and its gate are disposed, and a manual daemon winning the lock would leave the new KeepAlive job retrying with nobody left to roll it back.
...
| true | `installed`, `daemon_pid` null | `kcap daemon service start`, then **post-start verification** (below). A non-null `daemon_pid` means some process owns the name while the unit's job is down — starting would spin against the lock: surface the coexistence instead (§4.4 affordance). |
| false | `installed` (orphan label, no plist) | Nothing automatic. A loaded label without its unit file is a broken state (a failed bootout's leftover) — surfaced with the §4.4 affordance, never silently paved over. |
...
| false | `not_installed` (silent-mutation gate §4.1 passes) | `kcap daemon service install --name <X> --profile <P>` → **post-install verification**. |
| false | `not_installed` (gate fails) | Nothing. Today's Stopped UX (plus the §6 honest reason). The wizard (AI-1655) owns fresh machines. |

Rows are keyed on the loaded-label/job state **before** plist presence, same as §4.3/§4.4.
...
- Fresh `Connected` but ownership fails (a manual daemon won the lock) → for a unit **this operation just wrote**: `service uninstall` (rollback; the hardened uninstall makes a failed bootout visible instead of orphaning the job), log, stay attached to the manual daemon; skew handling (§4.3) applies to it as usual. For a **pre-existing** unit (post-start verification): do not destroy it silently — `service stop` (a true unload now, §3.4) restores the pre-operation inactive state, confirmed by a bounded re-query showing the label no longer loaded/retrying, and the coexistence is surfaced with the dialoged affordance.
...
| A **loaded label** exists that does not own the reachable daemon — loaded-or-stale unit + manual daemon coexisting, or an **orphan label** with no plist | `service uninstall` **first** — it clears the label whether or not a plist exists (benign-absence semantics, §3.4), and it must come first because a raw `daemon stop` sees the loaded label through the hardened status and no-ops → then `daemon stop --name <X>` if a live daemon still holds the name → **stop verification** → `service install`. |
| **No loaded label** (pure manual daemon) | `daemon stop --name <X>` → stop verification → `service install`. |

Branching is keyed on the loaded-label state before plist presence — `unit_present` alone never selects the sequence.
...
2. A loaded label exists (`state=installed`, plist or not): plist present and `daemon_pid` null → `service start` + post-start verification (§4.2); `daemon_pid` non-null (coexistence) or plist absent (orphan label) → the dialoged affordance — detached-starting past a loaded label would hand the name lock to a contender the label respawns against, turning an inactive orphan into a permanent lock fight.
3. No loaded label: plist present (present-but-unloaded) → the reinstall affordance; nothing at all → today's detached `daemon start -d` (detach-on-cancel semantics, §3.6).
...
- **Terminal-PATH probe unknown** → unit-writing mutations suppressed (silent ones) or disclosed (dialoged ones); starting an existing unit is exempt — it recaptures nothing (decision 7); queries unaffected.
...
- **CLI:** `service status --json` — the full grid: plist absent (with and without an orphaned loaded job — the hardened launchd query); plist present + print failing (`not_installed`, `unit_present=true`, non-null `binary_path`); loaded-inactive; running with `job_pid` parsed; `install_binary_path` present/missing. `daemon_pid` identity validation: absent/unusable file, dead PID, start-token mismatch (PID reuse), legacy token → name fallback — only a validated live owner emits non-null. Hardened `uninstall`: bootout non-zero + label confirmed absent → success, plist deleted; bootout non-zero + label still loaded → non-zero exit, plist retained. Hardened `stop`: bootout issued (not a signal), plist retained, post-stop status shows no job; hardened `start`: bootstraps when unloaded, kickstarts when loaded. snake_case contract; human output unchanged without `--json`.
...
- **Verification & ownership:** replayed `Connected` never satisfies verification; fresh `Connected` at wrong version fails it; fresh `Connected` + pid mismatch (manual daemon B owns lock, launchd job A transiently running) → correct rollback branch, manual daemon untouched (no `daemon stop` issued); deadline with job still `running` and no fresh attach → rollback; post-rollback re-query shows label gone, and an injected rollback failure is surfaced not swallowed; post-start (pre-existing unit) failure → `service stop` (unload) + bounded re-query proving the label is no longer loaded/retrying — never uninstall; auto-start of an existing unit proceeds with the probe unknown (decision 7 exemption).
...
- **Orphan labels (controller):** `unit_present=false` with `state=installed` and with `state=running`, through user Start (→ affordance, never detached start) and through takeover (→ uninstall-first clears the label; raw `daemon stop` is asserted absent from the argv sequence until the label is gone).
- **Races & shutdown:** status query resolving after `Connected` → discarded; user Start racing auto-install → serialized; dialog accept after generation change → abort; subscribe-before-pump; **shutdown during each mutating operation** (install, start, uninstall, takeover, reinstall) defers until verification/rollback completes — only detached `daemon start -d` abandons the wait.
- **Process/probe seams:** stdout returned; env overlay adds `KCAP_PROFILE`/`PATH` without replacing the environment; detach-on-cancel scoped to detached start (survives shutdown); internal timeout kills the whole tree, awaits exit, and forces a state reconcile before the next mutation (late-child-mutation test); `--version` parsing (prefix strip, multiline reject, `--no-update-check` in argv); probe `-lic` sentinel parse robust to chatter, `.zshrc`-only PATH found, `-lc` fallback, `$SHELL` unset → `/bin/zsh`, both fail → unknown semantics (silent mutations off, dialogs disclose).
