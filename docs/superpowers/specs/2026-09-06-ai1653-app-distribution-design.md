...
- `scripts/assert-app-cli-version.test.sh` pins the version comparison; `scripts/assert-bundle-digest.test.sh` the digest gate (§6.5); `scripts/promote-desktop-aliases.test.sh` the promotion rule (older candidate never regresses an alias; a beta never becomes the stable alias; a re-run is a no-op); `scripts/desktop-baseline.test.sh` the baseline rule (a lower full package is kept; an equal or higher one, such as a beta above a stable patch, is discarded; an empty download is a no-op); `scripts/verify-desktop-immutables.test.sh` the publication gate (absent objects pass, identical bytes pass, different bytes fail); `scripts/render-app-icons.sh` has no test (its outputs are committed and reviewed by eye).
