...
| 9 | **Update UX: background download, one prompt when ready, restart is the user's.** Automatic checks are silent on failure. After the relaunch the lifecycle slice's skew dialog offers the daemon restart; this slice adds no daemon logic and keeps the never-silent rule. |
...
After signing on the runner: run `kcap --version --no-update-check` and `kcap-daemon --version` from the signed files, and after `vpk pack` run the bundled `kcap` once more from inside the extracted bundle. A hardened-runtime or entitlement mistake crashes here, not on a user's machine. The daemon has no version flag today; this slice adds `--version`, handled before any config, environment or profile work, printing the same version string the CLI prints and exiting 0 — the one daemon change in the slice.

## 5. Feed and website

### 5.1 R2 layout

Bucket from the `R2_BUCKET` secret, prefix `desktop/osx-arm64/`. Per release: `releases.osx-arm64.json`, `KurrentCapacitor-<version>-osx-arm64-full.nupkg`, `KurrentCapacitor-<version>-osx-arm64-delta.nupkg` when a baseline exists, `Kurrent-Capacitor-<version>-osx-arm64.dmg`, `KurrentCapacitor-<version>-osx-arm64-Portable.zip`. Aliases overwritten by the workflow: `Kurrent-Capacitor-osx-arm64-beta.dmg` on every tag; `Kurrent-Capacitor-osx-arm64.dmg` on stable tags only (`scripts/npm-dist-tag.sh` already classifies a version as `beta`/`latest`; the same script decides). `--keepMaxReleases` is never passed: old packages stay, a user mid-download is never cut off.

Upload: `vpk upload s3 --endpoint "$R2_ENDPOINT" --bucket "$R2_BUCKET" --prefix desktop/osx-arm64 --keyId "$R2_ACCESS_KEY_ID" --secret "$R2_SECRET_ACCESS_KEY"` for Velopack's own outputs (it merges the manifest with what is already there), then the AWS CLI with `--endpoint-url "$R2_ENDPOINT"` for the DMG and the aliases. Before packing, `vpk download s3` with the same flags fetches the previous full package so the delta is generated; on an empty bucket it is allowed to fail with a logged line (a credentials failure surfaces again at upload, which is not tolerant).
...
- A Worker route `GET /download/desktop/*` served from an R2 binding on the bucket, mapping the URL path to the object key verbatim, with the object's content type; `Cache-Control: no-cache` on `releases.*.json`, `max-age=300` on the alias DMGs, `immutable` on versioned objects; 404 on a missing object.
...
- Matrix leg `osx-arm64` (steps conditioned on `startsWith(matrix.rid, 'osx-')`): keychain import; sign daemon + shim before the digest; sign the CLI after its publish; post-sign smoke.
- New job `app-macos` (`needs: [build, github-release]`, `runs-on: macos-latest`), in parallel with `publish-npm`: download `release-osx-arm64`, restore executable bits, keychain import + notary profile, `dotnet tool install -g vpk --version <pinned>`, publish the app, sign its binaries, assemble the pack dir, substitute the plist, `vpk download s3` (tolerant), `vpk pack` (§6.3), extract the bundle, `scripts/assert-app-cli-version.sh` (§6.4), in-bundle smoke, `scripts/build-dmg.sh`, sign/notarize/staple/assess the DMG, `vpk upload s3`, AWS CLI copies, and finally `gh release upload --clobber` of the DMG onto the GitHub Release that `github-release` has already created, so the DMG is discoverable from the repo too.
- Neither `publish-npm` nor `github-release` depends on `app-macos`: an app-side failure leaves the npm release and the GitHub Release intact (the signed binaries are already in their artifacts), and the app can be re-published by re-running the one job.

### 6.2 `ci.yml`

New job `app-bundle` on `macos-latest`, no secrets: restore, publish daemon and CLI for `osx-arm64` (with the digest handshake the release uses), publish the app, `vpk pack --noInst` without signing, `scripts/assert-app-cli-version.sh` against MinVer's computed version, run the bundled `kcap --version --no-update-check`, upload the portable zip as a workflow artifact (`app-osx-arm64-unsigned`). A reviewer runs it after `xattr -d com.apple.quarantine`; being a packed bundle it polls the real feed and, carrying a prerelease version, would accept a signed release as an update — expected, not a defect. The job's name joins the `check-regexp` that `release.yml`'s "Wait for CI" polls, so a tag never releases an app whose PR bundle job was red; adding it to branch protection is a repository setting outside this PR.
...
`VERSION` is the tag without build metadata (`vpk` requires plain SemVer2). On PRs the last line is omitted.
...
`VelopackApp.Build().Run()` is the first statement of `Program.Main`, before `BuildAvaloniaApp()`. Velopack runs install/update hooks and exits from inside `Run()`; anything placed before it would execute during those operations.
...
public interface IAppUpdater {
    ...
    void ApplyOnExit(UpdateCandidate candidate); // WaitExitThenApplyUpdates(asset, silent: false, restart: true)
}
```
...
- **Later:** the tray item reads "Restart to update to `<version>`" for the rest of the run and the prompt does not repeat for that version; Velopack applies the pending package at the next launch regardless.
- **Restart now** (or the tray item): call `ApplyOnExit`, then the app's existing quit path (`QuitCommand`), so daemon detach and teardown are ours and the swap plus relaunch is Velopack's.
...
The app now carries a newer CLI than the running daemon. AI-1654's skew detection (`Connected` with a differing daemon version) classifies the unit as **same-binary** (the plist's path equals the new install path) and offers "Restart daemon to update"; declining while agents run is remembered per `(daemonVersion, cliVersion)` as today. Agents survive the app's restart by construction (the daemon is a LaunchAgent). This slice adds no daemon-side logic.

### 7.5 Install-location guard

`InstallLocation.Classify(bundleRoot, home)` → `Applications | UserApplications | DmgVolume | Translocated | Other`, from the path only. Runs in `StartAsync` before the gate, only when `AppContext.BaseDirectory` is inside a `.app` bundle. `Applications` and `UserApplications` pass. Anything else shows a modal owned by the app (no daemon graph yet): "Move Kurrent Capacitor to your Applications folder to continue." with **Move to Applications** and **Quit**. Move copies the bundle with `ditto` to `/Applications/`, refuses when a bundle of that name already exists there (the dialog then says to open that copy), then `open -n` the copy and exits 0. Copy failure shows the error and keeps Quit.
...
- Release side, all fail closed and loud: empty signing or notary secret; bundled CLI version ≠ tag; version below floor; hardened-runtime crash in the post-sign smoke; notarization rejection (Velopack prints the notary log); `spctl` refusal; upload failure. Re-running `app-macos` is safe: uploads overwrite the same keys, and the manifest merge is idempotent for an existing version.
- Client side: the updater never blocks startup, never touches the daemon, and never surfaces an automatic failure; the guard never overwrites an existing bundle.
...
- `UpdateCoordinator` with a fake `IAppUpdater` and `FakeTimeProvider`: inert when unavailable; initial delay and interval; download-then-prompt; Later semantics and tray label; restart invokes `ApplyOnExit` exactly once then quit; manual check outcomes; coalescing.
- `InstallLocation.Classify` for the five shapes, including `~/Applications` and translocation paths.
- `scripts/assert-app-cli-version.test.sh` pins the comparison; `scripts/render-app-icons.sh` has no test (its outputs are committed and reviewed by eye).

**CI:** the `app-bundle` job (§6.2) is the automated end-to-end of bundling without signing.

**Manual E2E (accepted risk, umbrella §10):** on a clean Mac, install the beta DMG: Gatekeeper accepts it; the guard fires from the DMG volume and Move works; the application menu says "Kurrent Capacitor" with the new icon; the shim links into the bundle; the LaunchAgent starts the bundled daemon; `kcap update` from a terminal prints the bundled message. Cut the next beta: the app finds it, downloads, prompts, relaunches on the new version, and the skew dialog restarts the daemon. Verify the delta package was produced for the second release.

## 11. Scope boundaries

- **This PR:** everything in §3–§7, the daemon's `--version` flag (§4.4), `README.md` gains a "Desktop app" install section (DMG, first-run guard, where updates come from), `docs/CHANGES.md` gains an entry, help text unchanged except `kcap update`'s bundled message.
