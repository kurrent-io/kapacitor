...
- **`DaemonMutationLane`** — the singleton every daemon mutation runs through (wizard step 7,
  ...
  an enqueue racing the transfer is delivered to exactly one of them. Each envelope carries
  ...
  **Observation adapters** (two, chosen by startup mode): the live-graph adapter wraps
...
- **`DaemonMutationLane` (through its interface, all three callers):** wizard step 7 (no
  ...
  envelopes carry their `MutationRequest` identity for routing; a timed-out wrapper yields
